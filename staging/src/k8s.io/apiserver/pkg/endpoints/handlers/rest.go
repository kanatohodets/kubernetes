	...
	return func(w http.ResponseWriter, req *http.Request) {
		...
		if admit != nil && admit.Handles(admission.Connect) {
			...
			if mutatingAdmission, ok := admit.(admission.MutationInterface); ok {
				err = mutatingAdmission.Admit(admission.NewAttributesRecord(connectRequest, nil, scope.Kind, namespace, name, scope.Resource, scope.Subresource, admission.Connect, userInfo), &scope)
				if err != nil {
					scope.err(err, w, req)
					return
				}
			...
			if validatingAdmission, ok := admit.(admission.ValidationInterface); ok {
				err = validatingAdmission.Validate(admission.NewAttributesRecord(connectRequest, nil, scope.Kind, namespace, name, scope.Resource, scope.Subresource, admission.Connect, userInfo), &scope)
				if err != nil {
					scope.err(err, w, req)
					return
				}
